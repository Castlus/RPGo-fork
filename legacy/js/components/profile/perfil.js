 ...
 * @param {Object} [personagemInicial] - Personagem já carregado pela ficha (evita refetch)
 * @param {Object} [userInicial] - User da sessão Supabase já carregado (evita getUser remoto)
 */
...
export function carregarPerfil(uid, personagemInicial = null, userInicial = null) {
    function renderizarDados(dados) {
        ...
        // Mostrar User Email — usa o user passado pela ficha (sem round-trip)
        const elBtnInfo = document.getElementById('btnInfoUser');
        if (elBtnInfo && userInicial?.email) {
            elBtnInfo.title = `Logado como: ${userInicial.email}`;
    ...
    // Se o personagem já foi carregado pela ficha, renderiza direto; senão, busca.
    if (personagemInicial) {
        renderizarDados(personagemInicial);
    } else {
