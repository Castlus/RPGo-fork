 ...
 * @param {string} uid - ID do usuário (Supabase)
 * @param {Object} [personagemInicial] - Personagem já carregado pela ficha (evita refetch de cargaMaxima)
 */
export function carregarInventario(uid, personagemInicial = null) {
    inventarioState.uid = uid;

    // Carga Máxima — usa o personagem já carregado; senão, busca.
    if (personagemInicial) {
        const elMax = document.getElementById('maxPeso');
        if (elMax && personagemInicial.cargaMaxima != null) elMax.innerText = personagemInicial.cargaMaxima;
    } else {
