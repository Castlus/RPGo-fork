 ...
 * @param {Object} [personagemInicial] - Personagem já carregado pela ficha (evita refetch só pro nome)
 */
import { supabase, apiGet, apiPost, apiDelete } from '../../utils/api.js';

export function iniciarChat(user, SESSION_ID, personagemInicial = null) {
    // Nome do personagem — usa o já carregado, ou fallback pro email.
    let nomePersonagem = personagemInicial?.nome || user.email || 'Anônimo';
